...
- [ ] **事件极化改造**：很多事件仍有"明显最优解"或纯收益选项，应改为此消彼长的权衡（patch-14仅部分修复，patch-20的10个新事件自带极化但旧事件不改）→ 待 patch-23 spec
...
- [ ] 事件极化改造的优先级和范围（旧事件需逐步改造）→ 零和规则确认：基础四维(progress+morale+budget+bossTrust)≈0，qualityDebt/hidden不参与
